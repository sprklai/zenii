                    ...
                    Ok(AppEvent::SchedulerNotification {
                        ...
                        for target in channel_targets {
                            ...
                            #[cfg(feature = "channels")]
                            {
                                ...
                                let msg =
                                    ChannelMessage::new(&name, &formatted).with_sender("MesoClaw");
    ...
    pub fn format_message(event_type: &str, job_name: &str, detail: &str) -> String {
        match event_type {
            "scheduler_notification" => {
                format!("[MesoClaw] {job_name}: {detail}")
            }
            "scheduler_job_completed" => {
                format!("[MesoClaw] Job \"{job_name}\" completed — {detail}")
            }
            _ => {
                format!("[MesoClaw] {event_type}: {job_name} — {detail}")
            }
        }
    }
}
...
mod tests {
    ...
    fn format_message() {
        ...
        assert_eq!(msg, "[MesoClaw] daily_check: All OK");

        let msg =
            NotificationRouter::format_message("scheduler_job_completed", "backup", "success");
        assert_eq!(msg, "[MesoClaw] Job \"backup\" completed — success");

        let msg = NotificationRouter::format_message("unknown", "test", "detail");
        assert_eq!(msg, "[MesoClaw] unknown: test — detail");
    }
}
