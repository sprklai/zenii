...
impl PluginRegistry {
    ...
    pub fn new(plugins_dir: PathBuf) -> Result<Self> {
        std::fs::create_dir_all(&plugins_dir)
            .map_err(|e| MesoError::Plugin(format!("cannot create plugins dir: {e}")))?;
            ...
            && let Ok(index) = serde_json::from_str::<RegistryIndex>(&content)
        {
            for entry in index.plugins {
                ...
                let manifest_path = plugin_dir.join("mesoclaw-plugin.toml");
    ...
    fn scan_directory(&self) -> Result<()> {
        let entries = std::fs::read_dir(&self.plugins_dir)
            .map_err(|e| MesoError::Plugin(format!("cannot read plugins dir: {e}")))?;
        ...
        for entry in entries {
            ...
            let manifest_path = path.join("mesoclaw-plugin.toml");
    ...
    pub fn unregister(&self, name: &str) -> Result<()> {
        if self.plugins.remove(name).is_none() {
            return Err(MesoError::PluginNotFound(format!(
    ...
    pub fn save_index(&self) -> Result<()> {
        ...
        let content = serde_json::to_string_pretty(&index)
            .map_err(|e| MesoError::Plugin(format!("failed to serialize index: {e}")))?;

        let index_path = self.plugins_dir.join("registry.json");
        std::fs::write(&index_path, content)
            .map_err(|e| MesoError::Plugin(format!("failed to write index: {e}")))?;
...
"#
        ...
        std::fs::write(dir.join("mesoclaw-plugin.toml"), manifest).unwrap();
    }
...
description = "Persist test""#,
            ...
            std::fs::write(
                dir.path().join("persist/mesoclaw-plugin.toml"),
                r#"[plugin]
name = "persist"
version = "1.0.0"
description = "Persist test""#,
