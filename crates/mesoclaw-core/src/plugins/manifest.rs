...
pub struct PluginMeta {
    ...
    pub min_mesoclaw: Option<String>,
}
...
impl PluginManifest {
    ...
    pub fn parse(content: &str) -> Result<Self> {
        let manifest: Self = toml::from_str(content)
            .map_err(|e| MesoError::Plugin(format!("invalid manifest: {e}")))?;
        manifest.validate()?;
        Ok(manifest)
    }
    ...
    pub fn from_file(path: &Path) -> Result<Self> {
        let content = std::fs::read_to_string(path)
            .map_err(|e| MesoError::Plugin(format!("cannot read manifest: {e}")))?;
        Self::parse(&content)
    }
    ...
    fn validate(&self) -> Result<()> {
        if self.plugin.name.is_empty() {
            return Err(MesoError::Plugin("plugin name is required".into()));
        }
        if self.plugin.version.is_empty() {
            return Err(MesoError::Plugin("plugin version is required".into()));
        }
