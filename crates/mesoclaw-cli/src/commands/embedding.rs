...
pub async fn activate(client: &MesoClient, provider: &str) -> Result<(), String> {
    ...
    if provider == "local" {
        ...
        println!("Run 'mesoclaw embedding download' to trigger download now.");
...
pub async fn deactivate(client: &MesoClient) -> Result<(), String> {
    let config_update = serde_json::json!({
...
pub async fn download(client: &MesoClient) -> Result<(), String> {
...
pub async fn test(client: &MesoClient) -> Result<(), String> {
    let resp: serde_json::Value = client
...
pub async fn reindex(client: &MesoClient) -> Result<(), String> {
    let resp: serde_json::Value = client
...
mod tests {
    ...
    fn cli_embedding_status() {
        let cli = crate::Cli::parse_from(["mesoclaw", "embedding", "status"]);
        assert!(matches!(
            cli.command,
    ...
    fn cli_embedding_activate_local() {
        let cli = crate::Cli::parse_from(["mesoclaw", "embedding", "activate", "local"]);
        match cli.command {
            crate::Commands::Embedding {
    ...
    fn cli_embedding_deactivate() {
        let cli = crate::Cli::parse_from(["mesoclaw", "embedding", "deactivate"]);
        assert!(matches!(
            cli.command,
            crate::Commands::Embedding {
