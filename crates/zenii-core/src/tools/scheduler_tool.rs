    ...
    async fn execute(&self, args: serde_json::Value) -> Result<ToolResult> {
        ...
        match action {
            ...
            "delete" => {
                let id = args["job_id"]
                    .as_str()
                    .ok_or_else(|| ZeniiError::Validation("missing 'job_id' for delete".into()))?;
            ...
            "toggle" => {
                let id = args["job_id"]
                    .as_str()
                    .ok_or_else(|| ZeniiError::Validation("missing 'job_id' for toggle".into()))?;
            ...
            "history" => {
                let id = args["job_id"]
                    .as_str()
                    .ok_or_else(|| ZeniiError::Validation("missing 'job_id' for history".into()))?;
...
impl SchedulerTool {
    async fn create_job(&self, args: &serde_json::Value) -> Result<ToolResult> {
        ...
        let schedule_type = args["schedule_type"]
            .as_str()
            .ok_or_else(|| ZeniiError::Validation("missing 'schedule_type' for create".into()))?;
        let payload_type = args["payload_type"]
            .as_str()
            .ok_or_else(|| ZeniiError::Validation("missing 'payload_type' for create".into()))?;
