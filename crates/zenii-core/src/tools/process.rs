...
impl Tool for ProcessTool {
    ...
    async fn execute(&self, args: serde_json::Value) -> Result<ToolResult> {
        let action = args
            .get("action")
            .and_then(|v| v.as_str())
            .ok_or_else(|| ZeniiError::Tool("missing 'action' argument".into()))?;
        ...
        match action {
            "list" => {
                ...
                .await
                .map_err(|e| ZeniiError::Tool(format!("spawn error: {e}")))?;
...
mod tests {
    ...
    // 8.9.2 — kill without pid arg returns ZeniiError::Tool
    #[tokio::test]
    async fn kill_without_pid_returns_error() {
        ...
        assert!(matches!(err, crate::ZeniiError::Tool(_)));
    }
