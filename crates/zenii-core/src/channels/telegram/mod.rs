...
impl TelegramChannel {
    ...
    async fn handle_bot_command(&self, cmd: BotCommand, chat_id: ChatId) {
        ...
        let response = match cmd {
            BotCommand::Start => "Welcome to Zenii! Send me any message and I'll respond with AI assistance.".to_string(),
    ...
    async fn connect(&self) -> Result<()> {
        ...
        let token = self
            ...
            .map_err(|e| {
                ...
                ZeniiError::Channel(format!("telegram: credential error: {e}"))
            })?
            .ok_or_else(|| {
                self.status.store(STATUS_DISCONNECTED, Ordering::SeqCst);
                ZeniiError::Channel("telegram: bot token not configured".into())
            })?;
        ...
        let client = teloxide::net::default_reqwest_settings()
            ...
            .map_err(|e| {
                self.status.store(STATUS_DISCONNECTED, Ordering::SeqCst);
                ZeniiError::Channel(format!("telegram: failed to build HTTP client: {e}"))
            })?;
        ...
        let me = bot.get_me().await.map_err(|e| {
            self.status.store(STATUS_DISCONNECTED, Ordering::SeqCst);
            ZeniiError::Channel(format!("telegram: getMe failed: {e}"))
        })?;
    ...
    async fn listen(&self, tx: mpsc::Sender<ChannelMessage>) -> Result<()> {
        ...
        let bot = self
            ...
            .ok_or_else(|| {
                ...
                ZeniiError::Channel("telegram: not connected, call connect() first".into())
            })?
            .clone();
...
impl ChannelSender for TelegramSender {
    ...
    async fn send_message(&self, message: ChannelMessage) -> Result<()> {
        let bot = self
            .bot
            .as_ref()
            .ok_or_else(|| ZeniiError::Channel("telegram sender: not connected".into()))?;
