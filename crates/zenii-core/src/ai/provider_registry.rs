...
impl ProviderRegistry {
    ...
    pub async fn seed_builtin_providers(&self) -> Result<()> {
        let builtin: BuiltinProviders = serde_json::from_str(BUILTIN_PROVIDERS_JSON)
            .map_err(|e| ZeniiError::Config(format!("failed to parse providers.json: {e}")))?;
            ...
            let provider = conn
                ...
                .map_err(|e| match e {
                    rusqlite::Error::QueryReturnedNoRows => {
                        ZeniiError::NotFound(format!("provider not found: {id}"))
                    }
                    other => ZeniiError::from(other),
                })?;
    ...
    pub async fn add_user_provider(
        ...
        db::with_db(&self.db, move |conn| {
            ...
            .map_err(|e| match e {
                ...
                {
                    ZeniiError::Validation(format!("provider already exists: {id}"))
                }
                other => ZeniiError::from(other),
            })?;
    ...
    pub async fn update_provider(&self, id: &str, base_url: &str) -> Result<()> {
        ...
        db::with_db(&self.db, move |conn| {
            ...
            if rows == 0 {
                return Err(ZeniiError::NotFound(format!("provider not found: {id}")));
            }
            Ok(())
        })
        .await
        ...
        db::with_db(&self.db, move |conn| {
            ...
            .map_err(|e| match e {
                rusqlite::Error::SqliteFailure(ref err, _)
                    if err.code == rusqlite::ffi::ErrorCode::ConstraintViolation =>
                {
                    ZeniiError::Validation(format!("model already exists: {composite_id}"))
                }
                other => ZeniiError::from(other),
            })?;
    ...
    pub async fn delete_custom_model(&self, composite_id: &str) -> Result<()> {
        ...
        db::with_db(&self.db, move |conn| {
            let is_custom: bool = conn
                ...
                .map_err(|e| match e {
                    rusqlite::Error::QueryReturnedNoRows => {
                        ZeniiError::NotFound(format!("model not found: {composite_id}"))
                    }
                    other => ZeniiError::from(other),
                })?;
